...
export const ACTIONABLE_MESSAGE_TYPES = new Set(['handoff', 'question']);
export const TERMINAL_MESSAGE_TYPES = new Set(['note', 'result', 'ack']);
...
function messageBlock({ from, to, type, body, messageId, replyTo }) {
  const header = to ? `${from} → ${to}` : from;
  const id = messageId ? ` {#${messageId}}` : '';
  const replyMetadata = replyTo ? `\n\n<!-- reply-to: ${replyTo} -->` : '';
  const replyHint = ACTIONABLE_MESSAGE_TYPES.has(type) && messageId && from && to
    ? `\n\n<!-- reply with: ensemble send ${from} "<result>" --from ${to} --type result --reply-to ${messageId} -->`
    : '';
  return `\n## ${nowIso()} — ${header} [${type}]${id}\n\n${body.trim()}${replyMetadata}${replyHint}\n`;
}
...
export function send(root, { from = defaultAgent(), to, type = 'handoff', body, replyTo } = {}) {
  validateAgentName(from);
  validateAgentName(to);
  validateMessageType(type);
  if (replyTo) validateMessageId(replyTo);
  if (!body?.trim()) throw new Error('message body is required');
  ensureAgent(root, to);
  const messageId = createMessageId();
  append(path.join(ensembleDir(root), 'agents', to, 'inbox.md'), messageBlock({ from, to, type, body, messageId, replyTo }));
  const sent = audit(root, { action: 'send', from, to, type, body, messageId, replyTo: replyTo || undefined });
  if (replyTo && TERMINAL_MESSAGE_TYPES.has(type)) {
    audit(root, { action: 'done', from, messageId: replyTo, body: `auto-closed by ${messageId}` });
  }
  return sent;
}
...
function workspaceStatus(root) {
  ...
  const agents = fs.existsSync(agentsRoot)
    ? fs.readdirSync(agentsRoot, { withFileTypes: true }).filter(d => d.isDirectory() && !d.name.startsWith('.')).map(d => d.name)
    : [];
...
export function messages(root, { limit = 50, open = false } = {}) {
  ...
  for (const record of readAudit(root, { limit: 0 })) {
    ...
    if (record.action === 'send' && record.messageId) {
      const terminal = TERMINAL_MESSAGE_TYPES.has(record.type);
      index.set(record.messageId, {
        ...
        status: terminal ? 'done' : 'open',
        ackedBy: [],
        doneBy: terminal ? record.from : null,
        doneAt: terminal ? record.ts : null,
        doneBody: terminal ? 'terminal message type' : undefined,
        events: [{ action: 'send', ts: record.ts, from: record.from, replyTo: record.replyTo }],
      });
      if (record.replyTo) {
        const parent = index.get(record.replyTo) || {
          messageId: record.replyTo,
          ts: record.ts,
          from: null,
          to: null,
          type: null,
        ...
        parent.events.push({ action: 'reply', ts: record.ts, from: record.from, messageId: record.messageId, type: record.type });
        if (terminal && parent.status !== 'done') {
          parent.status = 'done';
          parent.doneBy = record.from;
          parent.doneAt = record.ts;
          parent.doneBody = `replied by ${record.messageId}`;
        }
        index.set(record.replyTo, parent);
      }
...
function summarizeAuditRecord(record) {
  ...
  if (record.action === 'retire') return `${record.by || '?'} retired ${record.agent}${record.pendingMessages ? ` (${record.pendingMessages} pending message(s) archived)` : ''}`;
...
export const RETIRED_DIRNAME = '.retired';

function lastActivityByAgent(root) {
  const seen = new Map();
  const mark = (agent, ts) => {
    if (!agent || !ts) return;
    const current = seen.get(agent);
    if (!current || String(ts) > String(current)) seen.set(agent, ts);
  };
  for (const record of readAudit(root, { limit: 0 })) {
    if (record.malformed) continue;
    if (record.action === 'send' || record.action === 'note' || record.action === 'ack' || record.action === 'done') {
      mark(record.from, record.ts);
    } else if (record.action === 'claim' || record.action === 'claim_update' || record.action === 'release'
      || record.action === 'inbox_read' || record.action === 'inbox_clear') {
      mark(record.agent, record.ts);
    } else if (record.action === 'retire') {
      mark(record.by, record.ts);
    } else if (record.action === 'init') {
      mark(record.actor, record.ts);
    }
  }
  return seen;
}

export function staleAgents(root, { days = 30 } = {}) {
  const cutoffMs = Date.now() - days * 86_400_000;
  const current = workspaceStatus(root);
  const seen = lastActivityByAgent(root);
  const claimsByAgent = new Map();
  for (const [targetPath, owner] of Object.entries(current.worktrees)) {
    if (!owner?.agent) continue;
    if (!claimsByAgent.has(owner.agent)) claimsByAgent.set(owner.agent, []);
    claimsByAgent.get(owner.agent).push(targetPath);
  }
  return current.agents
    .map(row => {
      const candidates = [seen.get(row.agent), row.lastReadAt, row.state?.since]
      ...
      return {
        ...
        idleDays: lastActiveMs === null ? null : Math.floor((Date.now() - lastActiveMs) / 86_400_000),
        pending: row.pending,
        claims: claimsByAgent.get(row.agent) || [],
      };
    })
    .filter(row => row.lastActiveAt === null || Date.parse(row.lastActiveAt) < cutoffMs)
    .sort((a, b) => String(a.lastActiveAt || '').localeCompare(String(b.lastActiveAt || '')));
}

export function retire(root, { agent, by = defaultAgent(), reason = '' } = {}) {
  validateAgentName(agent);
  validateAgentName(by);
  const agentsRoot = path.join(ensembleDir(root), 'agents');
  const dir = path.join(agentsRoot, agent);
  if (!fs.existsSync(dir)) throw new Error(`Unknown agent: ${agent}`);

  const worktreesFile = path.join(ensembleDir(root), 'worktrees.json');
  const worktrees = readJson(worktreesFile, {});
  const releasedClaims = [];
  for (const [targetPath, owner] of Object.entries(worktrees)) {
    if (owner?.agent !== agent) continue;
    delete worktrees[targetPath];
    releasedClaims.push(targetPath);
    audit(root, { action: 'release', agent, path: targetPath, previous: owner, force: true, via: 'retire' });
  }
  if (releasedClaims.length) writeJson(worktreesFile, worktrees);

  let pendingMessages = 0;
  try { pendingMessages = parseInboxMessages(fs.readFileSync(path.join(dir, 'inbox.md'), 'utf8')).length; } catch {}

  const retiredRoot = path.join(agentsRoot, RETIRED_DIRNAME);
  ensureDir(retiredRoot);
  const archivedTo = path.join(retiredRoot, `${agent}.${nowIso().replace(/[:.]/g, '-')}`);
  fs.renameSync(dir, archivedTo);
  return audit(root, { action: 'retire', agent, by, reason: reason.trim() || undefined, pendingMessages, releasedClaims, archivedTo });
}
